...
The repository addon and Kodi metadata are generated by GitHub Actions and
served from GitHub Pages. The `plugin.video.nzbdav` zip referenced by that
metadata comes from the latest stable, non-prerelease GitHub Release asset.
...
just ship              # Run tests then build release zip locally
...
just dist-clean        # Remove build artifacts + generated Pages/repo output
```
...
scripts/
  ...
  generate_repo.py       # Local/CI Pages repository generator
  select_stable_release.py # CI helper choosing latest stable GitHub Release
repo/
...
2. Update release notes in `CHANGELOG.md` and the Kodi-visible
   `repo/plugin.video.nzbdav/changelog.txt`
3. Run `just lint` and `just test`
4. Commit the docs/changelog/version changes
5. Tag and push: `git tag v0.X.0 && git push origin main v0.X.0`
6. The Release workflow uploads the `plugin.video.nzbdav` zip to the GitHub Release
7. The Pages workflow selects the latest stable, non-prerelease GitHub Release
   and publishes the generated repository at `https://ponchopig.github.io/nzbdavkodi`
