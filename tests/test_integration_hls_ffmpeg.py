...
def _force_teardown_hls_producer(producer):
    """Belt-and-braces teardown. ``producer.close()`` already sends
    SIGKILL to ffmpeg, but under pytest interruption (Ctrl-C, SIGKILL
    to pytest, mid-assertion exceptions) the teardown ordering matters:
    fire close() first, then re-verify the subprocess is gone and
    SIGKILL it again if not, and finally rmtree the session dir. This
    routine must be idempotent and swallow every expected failure so
    it can run unconditionally during fixture finalization."""
    try:
        producer.close()
...
@pytest.fixture
def hls_producer(served_test_mkv, tmp_path, request):
    """Construct an ``HlsProducer`` for the served test MKV, run
    ``prepare()``, yield the live producer, and guarantee SIGKILL-level
    teardown via pytest's fixture finalizer chain.

    Previously each test had its own ``try: producer = …; finally:
    producer.close()`` block. That leaves ffmpeg zombies and
    ``/tmp/nzbdav-hls-*`` orphans when pytest is interrupted between
    constructor and ``try`` statement. Letting pytest own teardown via
    ``yield`` + ``addfinalizer`` closes that window."""
    ...
    ctx = {
        "session_id": "integration-{}".format(request.node.name),
...
def test_hls_producer_real_ffmpeg_init_mp4_is_valid_iso_bmff(hls_producer):
