...
- For releases, bump only `repo/plugin.video.nzbdav/addon.xml`; GitHub Releases provide addon zips and GitHub Pages serves generated repository metadata.
...
- Do not commit generated repository artifacts or addon zips. Use `just repo` only for a local Pages/Kodi preview.
...
just release       # Build plugin.video.nzbdav zip locally
just ship          # test + build release zip locally
just repo          # Generate local Pages/Kodi preview in pages-dist/
just repo-zip      # Build local repo preview + copy repository zip to cwd
just clean         # Remove __pycache__, .pytest_cache, zip
just dist-clean    # clean + remove generated Pages/repo output
```
...
- `repo/repository.nzbdav/` -- Kodi repository addon descriptor pointing to Pages metadata
- `scripts/generate_repo.py` -- local/CI Pages repository generator
- `scripts/select_stable_release.py` -- CI helper choosing latest stable GitHub Release
...
- Release workflow triggers on `v*` tags: runs tests, verifies `addon.xml` version matches the tag, builds the addon zip, and uploads it to a GitHub Release.
- GitHub Pages at `https://ponchopig.github.io/nzbdavkodi` serves repository metadata generated by CI from the latest stable, non-prerelease GitHub Release asset.
...
6. Do not commit generated repository artifacts; `just repo` is only a local Pages/Kodi preview.
7. Run `just lint` and `just test`.
8. Commit and push to `main`.
9. Tag with the new semver and push the tag: `git tag vX.Y.Z && git push origin main vX.Y.Z`.

The Release workflow uploads the addon zip. The Pages workflow selects the
latest stable release and publishes the generated repository at the Pages URL.
